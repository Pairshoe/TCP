...
int out_of_order_size = false;
// which packet will be dropped
int packet_drop_num_1 = 0;  // 11
int packet_drop_num_2 = 0;  // 13
int packet_num = 0;
...
ssize_t recv_packet(uint8_t *buffer, size_t buffer_size) {
  ...
  if (size >= 0) {
    ...
    packet_num++;
    if (packet_num == packet_drop_num_1 ||
        packet_num == packet_drop_num_2) {
      printf("Recv packet dropped\n");
      return -1;
    } else {
      // pcap
...
void parse_argv(int argc, char *argv[]) {
  ...
  // parse arguments
  while ((c = getopt(argc, argv, "hl:r:t:p:R:S:c:s:o:")) != -1) {
    switch (c) {
    case 'h':
      hflag = 1;
      break;
    ...
    case 's':
      ...
      break;
    case 'o':
      out_of_order_size = atoi(optarg);
      printf("Out of Order Flag = %d\n", out_of_order_size);
      break;
    case '?':
